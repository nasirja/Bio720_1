---
title: "JN_Bio720_R_InClassExercise2"
author: "Jalees Nasir"
...
The first loop initializes the variable 'x' before the loop, which is then updated via the loop.

The second loop is part of a function. The variable is only present and active when the function is initialized. 'x' cannot be printed outside the function.
...
```{R}
vec <- c()
system.time(for (i in 1:10000){vec[i] <- i^2})
```
...
```{R}
vec2 <- numeric(10000)
system.time(for (i in 1:10000){vec2[i] <- i^2})
```
Pre-allocating is faster as it only needs to load the vector once. The first example has you reload the information into memory instead of already having the template loaded into memory.
...
R treats the elements within a vector individually (it is vectorized). This means we can square each element in the vector by squaring the vector.
